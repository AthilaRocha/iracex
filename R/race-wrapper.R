...
race.init <- function(ids, idsagg,candidates, candidatesagg, parameters, config,media,effects)
{
...
race.info <- function(data)
  return(list(race.name = "irace",no.candidates = data$no.candidates, 
...
race.wrapper <- function(candidate, task, alive, data,testtype)
{
  ...
  if (testtype==1) cnd <- data$candidates[candi, , drop = FALSE]
  else cnd <- data$candidatesagg[candi, , drop = FALSE]
  
  for (i in seq_len (data$parameters$nbParameters)) {
    name <- data$parameters$names[[i]]
    values[[name]] <- cnd[[name]]
  }
  candidates <- vector("list",1)
  candidates<-list(index=candi, values=values,labels=data$parameters$switches)
  
  
#   if (candidate == alive[1]) {
#     print("entrou no if do candidate==alive[1]")
#     candidates <- vector("list", length(alive))
#     for (k in seq_along(alive)) {
#       candi <- alive[k]
#       values <- data$parameters$names
#       if (testtype==1) cnd <- data$candidates[candi, , drop = FALSE]
#       else cnd <- data$candidatesagg[candi, , drop = FALSE]
...
# 
#     # Execute commands
#     #.irace$hook.output <- vector("list", length(alive))
    
    .irace$hook.output <- vector("list", 1)
    cwd <- setwd (execDir)
    ...
    if (!is.null(data$config$hookRunParallel)) {
      ...
      #for (k in seq_along(candidates)) {
        ##print("Instance avaliation...\n")
        ##print(instance)
        .irace$hook.output[[1]] <- runif(1)*10000000
        #.irace$hook.output[1] <- .irace$hook.run(candidates[1],instance = instance, extra.params = extra.params,
         #                 config = data$config)
        ##In this simulation the lines above don't work
  ...
  #output <- .irace$hook.output[[match(candidate, alive)]] 
  
  output <- .irace$hook.output[[1]] 
  ...
  ##minha mudança (02/10/15)
  output <-as.matrix(output)
  output <-cbind(output,c(instance.idx))
